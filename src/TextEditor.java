...
public class TextEditor {
    ...
    public TextEditor(Assembler obj) {
        ...
        // Apply dark editor defaults immediately so the pane is never a blank white box
        jTextPane1.setBackground(new java.awt.Color(0x1E, 0x1E, 0x1E));
        jTextPane1.setForeground(new java.awt.Color(0xD4, 0xD4, 0xD4));
        jTextPane1.setCaretColor(java.awt.Color.WHITE);
        jTextPane1.setSelectedTextColor(java.awt.Color.YELLOW);
        jScrollPane1.setBackground(new java.awt.Color(0x1E, 0x1E, 0x1E));
        jScrollPane1.getViewport().setBackground(new java.awt.Color(0x1E, 0x1E, 0x1E));
    ...
    public void setColor(Color c, String s) {
        try {
            ...
            javax.swing.text.Style style = jTextPane1.getStyle("reusable");
            if (style == null) {
                style = jTextPane1.addStyle("reusable", null);
            }
    ...
    public void jTextPane1KeyReleased(java.awt.event.KeyEvent evt) {
        try {
            ...
            System.err.println("Exception in key release: " + t);
        }
    }
    ...
    public void runLinting() {
        try {
            ...
            System.err.println("Linting failed: " + t);
        }
    }
    ...
    public void colorEditor() {
        ...
        try {
            // Determine default text color by actual background luminance.
            Color bg = jTextPane1.getBackground();
            if (bg == null)
                bg = Color.BLACK;
            ...
            for (int i = s.length() - 2; i >= 0; i--) {
                ...
                if (label && (s.charAt(i) == '#' || s.charAt(i) == '.' || s.charAt(i) == ';' || s.charAt(i) == '/'
                        || s.charAt(i) == '\n')) {
                    label = false;
            ...
            // Replace with literal replaces and null checks
            if (p != null && p.code != null) {
                for (int i = 0; i < p.code.length; i++) {
                    if (p.code[i] != null && !p.code[i].equalsIgnoreCase("")) {
                        if (s.contains(p.code[i])) {
                            s = s.replace(p.code[i], (char) (0x200) + p.code[i] + (char) (0x201));
                        }
                        if (s.contains(p.code[i].toLowerCase())) {
                            s = s.replace(p.code[i].toLowerCase(),
                                    (char) (0x200) + p.code[i].toLowerCase() + (char) (0x201));
                        }
                    }
                }
            }
            ...
            if (code != null) {
                for (int i = 0; i < code.length; i++) {
                    if (code[i] != null && !code[i].equalsIgnoreCase("")) {
                        if (s.contains(code[i])) {
                            s = s.replace(code[i], (char) (0x205) + code[i] + (char) (0x206));
                        }
                        if (s.contains(code[i].toLowerCase())) {
                            s = s.replace(code[i].toLowerCase(),
                                    (char) (0x205) + code[i].toLowerCase() + (char) (0x206));
                        }
                    }
                }
            }
            ...
            for (int i = 0, mode = 0; i < s.length(); i++) {
                ...
                if (s.charAt(i) != 0x205 && s.charAt(i) != 0x206 && s.charAt(i) != 0x207 && s.charAt(i) != 0x200
                        && s.charAt(i) != 0x201) {
            ...
            System.err.println("colorEditor failed: " + t);
            t.printStackTrace();
            try {
                // Fallback: insert the text raw with the default style
                StyledDocument doc = jTextPane1.getStyledDocument();
    ...
    public void highligher(int row) {
        ...
        try {
            ...
            jTextPane1.getHighlighter().addHighlight(line[row], line[row + 1],
                    new DefaultHighlighter.DefaultHighlightPainter(new Color(0xFF3333)));
        } catch (Exception ex) {
